...
double delta_fii_i = 0.0 * Pi/180;
//
int pol = 4; //0 - lineaara; 1 = pa labi; -1 = pa kreisi
double teta_i = Pi/2 + delta_teta_i; //Pi/2; //
double fii_i = 0 + delta_fii_i;
//
// ierosmes noskanjoshana	//SVARIGI!!!!!!!
int Fg_d = 4;// 4; //3;//4; //1; //2; //1; //
int Fe_d = 4; //3; //3; //3;//1; //1; //3; //1; //
//
//
...
double delta_teta = delta_teta_i;
double delta_fii = delta_fii_i;

int int_phase_deg[2] = {100, 100};
int int_alpha_deg[2] = {45, 0};

double global_phase[2] = {int_phase_deg[0]*Pi/180, int_phase_deg[1]*Pi/180}; // Pi/2; // darbojas definējot E vektoru, ja 1/sqrt(2)((cosa-exp(i*phase)*sina)*x + (sina+exp(i*phase)*cosa)*y)
double global_alpha[2] = {int_alpha_deg[0]*Pi/180, int_alpha_deg[1]*Pi/180}; //0; // darbojas definējot E vektoru, ja 1/sqrt(2)((cosa-exp(i*phase)*sina)*x + (sina+exp(i*phase)*cosa)*y)
//
int pol_o[2] = {pol, -pol};  //{0, 0}; //0 - lineaara; 1 - pa labi; -1 - pa kreisi //pol_o[2] nozime ka output bus 2 komponetnes
double teta_o[2] = {teta_i + delta_teta, teta_i + delta_teta}; //ar komatu atdala pirmo un otro komponenti
double fii_o[2] = {fii_i + delta_fii, fii_i + delta_fii}; //{0, Pi/2}; //
//
//zondējošā stara ģeometrija	//SVARIGI!!!!!!!
//
int pol_p[2] = {pol, -pol};  //{0, 0}; //0 - lineaara; 1 - pa labi; -1 - pa kreisi //pol_o[2] nozime ka output bus 2 komponetnes
double teta_p[2] = {teta_i + delta_teta, teta_i + delta_teta}; //{0, Pi/2}; // ar komatu atdala pirmo un otro komponenti
double fii_p[2] = {fii_i + delta_fii, fii_i + delta_fii}; //{0, Pi/2}; //{0, Pi/2}; //{Pi, Pi}; //{Pi, Pi}; //
...
*/
    if(abs(q) < 1.5) // ja q par lielu, tad nav jeegas neko dariit
    {
        ...
        else if(pol_u == -2) // Pa labi polarizeets 1/sqrt(2)(x + exp(i*phi)*y); phi = Pi => 1/sqrt(2)(x - y)
        {
            gsl_vector_complex_set(initial, 0, gsl_complex_rect(1/sqrt(2), 0));
            gsl_vector_complex_set(initial, 1, gsl_complex_rect(-1/sqrt(2), 0));
        }
        else if(pol_u == -3) // Pa labi polarizeets 1/sqrt(2)(((cosa+sina)*x + exp(i*phi)*(cosa+sina)*y); a = Pi/4; phi = Pi => 1/sqrt(2)(x - y)
        {
            gsl_vector_complex_set(initial, 0, gsl_complex_rect(1, 0));
            // gsl_vector_complex_set(initial, 1, gsl_complex_rect(-1/sqrt(2), 0));
        }
        else if(pol_u == 4) //  1/sqrt(2)((cosa-exp(i*phase)*sina)*x + (sina+exp(i*phase)*cosa)*y); a = Pi/4; phi = Pi => 1/sqrt(2)(x - y)
        {
        	// double
            gsl_vector_complex_set(initial, 0, gsl_complex_rect(1/sqrt(2)*(cos(global_alpha[0])-cos(global_phase[0])*sin(global_alpha[0])), -1/sqrt(2)*sin(global_phase[0])*sin(global_alpha[0])));
            gsl_vector_complex_set(initial, 1, gsl_complex_rect(1/sqrt(2)*(sin(global_alpha[0])+cos(global_phase[0])*cos(global_alpha[0])), 1/sqrt(2)*sin(global_phase[0])*cos(global_alpha[0])));
        }
        else if(pol_u == -4) //  1/sqrt(2)((cosa-exp(i*phase)*sina)*x + (sina+exp(i*phase)*cosa)*y); a = Pi/4; phi = Pi => 1/sqrt(2)(x - y)
        {
        	// double
            gsl_vector_complex_set(initial, 0, gsl_complex_rect(1/sqrt(2)*(cos(global_alpha[1])-cos(global_phase[1])*sin(global_alpha[1])), -1/sqrt(2)*sin(global_phase[1])*sin(global_alpha[1])));
            gsl_vector_complex_set(initial, 1, gsl_complex_rect(1/sqrt(2)*(sin(global_alpha[1])+cos(global_phase[1])*cos(global_alpha[1])), 1/sqrt(2)*sin(global_phase[1])*cos(global_alpha[1])));
        }
...
//		0.5,	// 1
	...
	cout << "E+1: (" << GSL_REAL(E(1, pol_o[0], teta_o[0], fii_o[0])) << "," << GSL_IMAG(E(1, pol_o[0], teta_o[0], fii_o[0])) << ")"
		...
		<< endl;
	cout << "noveeroshana: pol -> " << pol_o[1] << "; teta -> " << teta_o[1] << "; fii -> " << fii_o[1] << endl;
	...
	lShift = 24.9; //-5316; //-1919; // 0; // -2280; //0; // //Izskanosana no parejas, vienibas - MHz
	...
	for(int Ri = 0; Ri <= 2; Ri += 1){ //sheit izvelas un ievieto vajdzigos rabi indeksus, kas atbilsti rabi frekvencem kas definetas augstak/ Taisa rabi ciklus sakas ar Ri= un beigu vertiba Ri< un Ri+= saka par cik palielinat katru nakamo
	...
	dataFile.open(filename, ios::out | ios::trunc);
	//Absorption file
	sprintf(fldname2, "%s%s%.2f%s%.2f%s%.2f%s%.2f%s%i%s%i%s%i%s%i%s%i%s%i%s%.4f%s%.1f", fldname, "/Absorption_theta=", teta_p[0],"-",teta_p[1], "_phi=", fii_p[0],"-",fii_p[1],"_pol=",pol_p[0],"-",pol_p[1],"_ph=",int_phase_deg[0],"-",int_phase_deg[1],"_a=",int_alpha_deg[0],"-",int_alpha_deg[1],"_gammaprobe=",Gamma_mazs_probe,"_lWidthpr=", liin_platums_probe);
	if(!FileExists(fldname2)){ cout << "creating folder " << fldname2 << endl; mkdir(fldname2, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH); }

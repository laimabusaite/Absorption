...
double delta_teta = 5.0 * Pi/180;
double delta_fii = 5.0 * Pi/180;

double global_phase[2] = {100*Pi/180, -100*Pi/180}; // Pi/2; // darbojas definējot E vektoru, ja 1/sqrt(2)((cosa-exp(i*phase)*sina)*x + (sina+exp(i*phase)*cosa)*y)
double global_alpha[2] = {-40*Pi/180, 0}; //0; // darbojas definējot E vektoru, ja 1/sqrt(2)((cosa-exp(i*phase)*sina)*x + (sina+exp(i*phase)*cosa)*y)
//
int pol_o[2] = {0, 0};  //{0, 0}; //0 - lineaara; 1 - pa labi; -1 - pa kreisi //pol_o[2] nozime ka output bus 2 komponetnes
double teta_o[2] = {0 + delta_teta, Pi/2 + delta_teta}; //ar komatu atdala pirmo un otro komponenti
double fii_o[2] = {0 + delta_fii, Pi/2 + delta_fii}; //{0, Pi/2}; //
//
...
double teta_p[2] = {0 + delta_teta, Pi/2 + delta_teta}; //{0, Pi/2}; // ar komatu atdala pirmo un otro komponenti
double fii_p[2] = {0 + delta_fii, Pi/2 + delta_fii}; //{0, Pi/2}; //{0, Pi/2}; //{Pi, Pi}; //{Pi, Pi}; //
...
int main(){
	...
	for(int Ri = 1; Ri <= 4; Ri += 1){ //sheit izvelas un ievieto vajdzigos rabi indeksus, kas atbilsti rabi frekvencem kas definetas augstak/ Taisa rabi ciklus sakas ar Ri= un beigu vertiba Ri< un Ri+= saka par cik palielinat katru nakamo
